...
class Checker extends Component {
    ...
    render() {
        ...
        const addServiceHandler = (service) => {
            this.setState({ loading: true, info: null });
            addCheckerService(this.state.checker.id, service.id)
                .then(() => {
                    this.setState({
                        loading: false,
                        checkerServices: this.state.checkerServices
                            .concat({ checker: this.state.checker.id, service })
        ...
        const removeServiceHandler = (checkerService) => {
            this.setState({ loading: true, info: null });
            removeCheckerService(this.state.checker.id, checkerService.service.id)
                .then(() => {
                    this.setState({
                        loading: false,
                        checkerServices: this.state.checkerServices.filter(
                            (service) => service.service.id !== checkerService.service.id
                        )
                    });
                })
                .catch((error) => this.setState({ loading: false, info: error }));
        ...
        const nonAddedServices = this.state.services?.filter(
            (service) => !this.state.checkerServices?.some((checkerService) => checkerService.service.id === service.id)
        );
        ...
        return (
            <div className="checker-page">
                ...
                <div>Services:</div>
                {!!this.state.checkerServices?.length && (
                    <table>
                        <thead>
                            <tr>
                                <th>Service</th>
                        ...
                        <tbody>
                            {this.state.checkerServices?.map((service) => (
                                <tr key={service.service.id}>
                                    <td>
                                        <Link to={"/services/" + service.service.id}>{service.service.name}</Link>
                                    </td>
                                    <td>
                                        {
                                            {
