...
class Checker extends Component {
    ...
    render() {
        const editHandler = () => {
            const updates = {};
            if (this.nameInputRef.current.value.trim() !== this.state.checker.name) updates.name = this.nameInputRef.current.value;
            if (this.descriptionInputRef.current.value.trim() !== this.state.checker.description) updates.description = this.descriptionInputRef.current.value;
            if (this.locationInputRef.current.value.trim() !== this.state.checker.location) updates.location = this.locationInputRef.current.value;
            if (parseInt(this.checkSecondInputRef.current.value) !== this.state.checker.checkSecond) updates.checkSecond = parseInt(this.checkSecondInputRef.current.value);
            ...
            updateChecker(this.state.checker.id, updates)
                .then(() => this.setState({ loading: false, editing: false, checker: { ...this.state.checker, ...updates } }))
                .catch((error) => this.setState({ loading: false, info: error }));
        };
            ...
            <div className="checker-page">
                ...
                {!this.state.editing ? <>
                    ...
                    <div className="input-field">
                        <div>Name:</div>
                        <input ref={this.nameInputRef} defaultValue={this.state.checker.name} disabled={this.state.loading} autoFocus
                            onKeyDown={(event) => event.key === "Enter" && this.descriptionInputRef.current.focus()} />
                    </div>
                    <div className="input-field">
                        <div>Description:</div>
                        <input ref={this.descriptionInputRef} defaultValue={this.state.checker.description} disabled={this.state.loading}
                            onKeyDown={(event) => event.key === "Enter" && this.locationInputRef.current.focus()} />
                    </div>
                    <div className="input-field">
                        <div>Location:</div>
                        <input ref={this.locationInputRef} defaultValue={this.state.checker.location} disabled={this.state.loading}
                            onKeyDown={(event) => event.key === "Enter" && this.checkSecondInputRef.current.focus()} />
                    </div>
                    <div className="input-field">
                        <div>Check second:</div>
                        <input ref={this.checkSecondInputRef} type="number" min="0" max="59" defaultValue={this.state.checker.checkSecond} disabled={this.state.loading}
